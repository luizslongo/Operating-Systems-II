...
// Class attributes
const char * Cortex_USB::_send_buffer = reinterpret_cast<const char *>(0);
unsigned int Cortex_USB::_send_buffer_size = 0;

// Class methods
void Cortex_USB::disable()
{
    reg(CTRL) &= ~USBEN;
...
unsigned int Cortex_USB::get(char * out, unsigned int max_size)
{
    lock();
    input();
    while(!(reg(CSOL) & CSOL_OUTPKTRDY));
...
void Cortex_USB::put(char c)
{
    if(Traits<Cortex_USB>::blocking) {
        while(!ready_to_put());
    } else {
        if(!ready_to_put())
            return;
...
void Cortex_USB::put(const char * c, unsigned int size)
{
    if(Traits<Cortex_USB>::blocking) {
        while(!ready_to_put());
    } else {
...
bool Cortex_USB::handle_ep0(const USB_2_0::Request::Device_Request & data)
{
    switch(data.bRequest)
    {
        ...
        case CDC::GET_LINE_CODING:
            if(auto d = data.morph<CDC::Request::Get_Line_Coding>())
            {
                ...
                data.dwDTERate = 1/Traits<Cortex_UART>::DEF_BAUD_RATE;
                switch(Traits<Cortex_UART>::DEF_STOP_BITS)
                {
                    case 1: data.bCharFormat = 0; break;
                    case 2: data.bCharFormat = 2; break;
                    default: return false;
                }
                data.bParityType = Traits<Cortex_UART>::DEF_PARITY;
                data.bDataBits = Traits<Cortex_UART>::DEF_DATA_BITS;
...
void Cortex_USB::int_handler(const IC::Interrupt_Id & interrupt)
{
    Reg32 index = endpoint(); // Save old index

    //db<Cortex_USB>(TRC) << "Cortex_USB::int_handler" << endl;
    //db<Cortex_USB>(TRC) << "CS0_CSIL = " << reg(CS0_CSIL) << endl;
    //db<Cortex_USB>(TRC) << "CIF = " << cif << endl;
    //db<Cortex_USB>(TRC) << "OIF = " << reg(OIF) << endl;
    //db<Cortex_USB>(TRC) << "IIF = " << reg(IIF) << endl;
    ...
    if((flags = reg(CIF))) // USB interrupt flags are cleared when read
    {
        if(flags & INT_RESET)
            reset();
        db<Cortex_USB>(TRC) << "CIF = " << flags << endl;
    }
    ...
    if((flags = reg(IIF))) // USB interrupt flags are cleared when read
    {
        if(flags & (1 << 0)) // Endpoint 0 interrupt
        {
            ...
            {
                db<Cortex_USB>(TRC) << "Endpoint 0 command received:";
                ...
                {
                    data[i] = reg(F0);
                    db<Cortex_USB>(TRC) << " " << (int) data[i];
                }
                db<Cortex_USB>(TRC) << endl;

                db<Cortex_USB>(TRC) << data << endl;
                ...
                if(handle_ep0(data))
                {
                    ...
                    db<Cortex_USB>(TRC) << "command processed" << endl;
                }
                else
                {
                    // Signal that the command could not be executed
                    reg(CS0_CSIL) |= CS0_CLROUTPKTRDY | CS0_SENDSTALL;
                    db<Cortex_USB>(WRN) << "Cortex_USB::int_handler: command NOT processed" << endl;
                }
            }
        ...
        db<Cortex_USB>(TRC) << "IIF = " << flags << endl;
    }
    if((flags = reg(OIF))) // USB interrupt flags are cleared when read
    {
        db<Cortex_USB>(TRC) << "OIF = " << flags << endl;
    }
