...
inline static void page_tables_setup() {
    ...
    for(int curr_page = 4095; curr_page > 0; curr_page--) {
        ...
        reinterpret_cast<volatile CPU::Reg32 *>(Memory_Map::FLAT_PAGE_TABLE)[curr_page] = aux;
    }
    reinterpret_cast<volatile CPU::Reg32 *>(Memory_Map::FLAT_PAGE_TABLE)[0] = TTB_DESCRIPTOR;
}
...
inline static void enable_mmu() {
    ...
    ASM ("MCR p15, 0, %0, c2, c0, 0" : : "p"(Memory_Map::FLAT_PAGE_TABLE) :); // Write Translation Table Base Register 0.
...
void Realview_PBX::pre_init()
{
    ...
    ASM("MCR p15, 0, %0, c12, c0, 0" : : "p"(Memory_Map::VECTOR_TABLE) :);
    ...
    if(CPU::id() == 0) {
        ...
        ASM("str %0, [%1]" : : "p"(Memory_Map::VECTOR_TABLE), "p"(FLAG_SET_REG) :);
