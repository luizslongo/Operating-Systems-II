...
void _entry() // machine mode
{
    ...
    if(CPU::mhartid() == 0)                             // SiFive-U has 5 cores, but core 0 (an E51) does not feature an MMU, so we halt it and let core 1 (an U54) run in a single-core configuration
        CPU::halt();
