...
{   //                                       p,               d, c,             act, t
    ...
    */
    ...
    Thread* e = new Thread(&my_func_aperiodic, 'C');
    Thread* h = new Thread(&my_func_aperiodic, 'D');
    Thread* f = new Thread(&my_func_aperiodic, 'E');
    Thread* g = new Thread(&my_func_aperiodic, 'F');
    Periodic_Thread* c = new Periodic_Thread(RTConf(period_g * 1000, period_g * 1000, wcet_g * 1000, 0, iterations, EDF_Modified::BEST_EFFORT)   , &my_func, 'G', (int)2.5e7);
    Periodic_Thread* d = new Periodic_Thread(RTConf(period_h * 1000, period_h * 1000, wcet_h * 1000, 0, iterations, EDF_Modified::BEST_EFFORT)   , &my_func, 'H', (int)1e7);    
