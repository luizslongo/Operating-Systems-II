...
class AC_Komeco_Control {
	public:
	AC_Komeco_Control() : _gpio(13), data0_mask(0) 
	{ 
		ac_params[0] = Middle;
	...
	void ac_onoff(char value) {
		kout << "ac_onoff " << hex << (int)value << dec << endl;
		if(value) set_temp();
		else turn_off();
	}

	void ac_temperature(char value) {
		kout << "ac_temperature " << hex << (int)value << dec << endl;
	...
	const static char _temperature[15];
...
int gateway() {
	...
	CPU::out32(IO::GPIO_PAD_DIR0, 0u | (1 << 12));
