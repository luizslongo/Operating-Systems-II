    ...
    private:
        ...
        class CompareByDeadline {
            ...
            friend bool operator<(Thread& a, Thread& b) {
                // If the current thread is CRITICAL, and the other is not, then it has less priority 
                if (a.type() == Thread::CRITICAL && b.type() != Thread::CRITICAL)
                    return false;
                // In case both thread are the same type, compares them by deadline (shortest deadline has priority)
        ...
        // Ordered queue of threads, that organizaes them based on the criteria defined on the class "CompareByDeadline"
