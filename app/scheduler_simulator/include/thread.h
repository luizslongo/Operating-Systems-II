...
class Thread {
public:
  // Enumeration of the thread types: CRITICAL (absolutely can NOT lose its
  // deadline) and BEST_EFFORT (losing its deadline will not cause major issues)
  enum type : int { CRITICAL = 0, BEST_EFFORT = 1 };
  // Thread constructor
  Thread(ThreadArgs &args)
      : _id{args.id}, _type(args.type), _execution_time(0),
  ...
  // Calculates the new CPU frequency based on the current time, and the minimum
  // and maximum allowed frequencies
...
private:
  ...
  // The following methods exist to deal with EPOS limitations (exponentiation
  // in which the exponent is a float) This absolute value calculation is used
  // in the method below
  float abs(float x) const { return x < 0 ? -x : x; }

  int round(float x) const { return x - (int)x < 0.5 ? (int)x : (int)x + 1; }

  // Calculates the power of 2 using the bisection method
  float pow2(float x) const {
    // Assuming we are only going to call this with y <= 1.0, so r = 2.1 is
    // enough (it should never be higher than 2)
    float l = 0, r = 2.1;
    ...
    while (abs(r - l) >= 1e-5) {
      ...
      // TODO: This is wrong for some reason... It does not work inside EPOS,
