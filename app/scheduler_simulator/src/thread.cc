...
int Thread::type() { return _type; }

// Returns the thread deadline
unsigned int Thread::deadline() { return _deadline; }

// Returns the thread creation time
unsigned int Thread::creation_time() { return _creation_time; }

// Returns the reference to the thread element in the ordered queue (used by the
// scheduler)
EPOS::Ordered_Queue<Thread>::Element &Thread::element() { return _element; }

// Function resposible for calculating the new CPU frequency needed for the
...
float Thread::new_frequency(unsigned int current_time, float min_cpu_frequency,
  ...
  float time_fraction =
      1.0 - static_cast<float>((_deadline - current_time)) /
                static_cast<float>((_deadline - _creation_time));
  float deadline_multiplier;

  // In case more than 80% of the time has passed, the multiplier will be 1.0
  // (CPU will operate in maximum frequency)
  if (time_fraction > 0.8) {
    deadline_multiplier = 1.0;
  }
  // Otherwise, applies a non-linear multiplier to accelerate the thread
  // execution based on the time fraction (the CPU frequency will increase the
  // more time passes, untill reaching a maximum frequency)
  else {
  ...
  // will never be above maximum and below minimum frequencies
  float new_cpu_frequency =
      min_cpu_frequency +
      deadline_multiplier *
...
// Verifies if the thread has already finished its execution (its execution time
// will be greater than the total task time)
bool Thread::finished() {
  EPOS::OStream cout;
  cout << "(Theread " << _id << ") " << " --> " << "TASK: " << _task_time
       << ", EXEC: " << _execution_time << ", ID: " << _id << '\n';
