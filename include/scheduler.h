...
public:
  ...
  */
  ...
  // Constructor helpers
  enum : unsigned int { SAME = 0, NOW = 0, UNKNOWN = 0, ANY = -1U };

  // Policy types
  enum : int { PERIODIC = PHIGH, SPORADIC = PLOW, APERIODIC = LOW };

  // Task types
  enum : int { CRITICAL = PHIGH, BEST_EFFORT = PNORMAL };
  ...
  union Dummy_Statistics { // for Traits<System>::monitored = false
    ...
    Tick thread_last_dispatch;   // tick in which the thread was last dispatched
                                 // to the CPU
    Tick thread_last_preemption; // tick in which the thread left the CPU by the
                                 // last time

    // Job related statistics
    bool job_released;
    Tick job_release; // tick in which the last job of a periodic thread was
                      // made ready for execution
    Tick job_start;   // tick in which the last job of a periodic thread started
                    // (different from "thread_last_dispatch" since jobs can be
                    // preempted)
    Tick job_finish; // tick in which the last job of a periodic thread finished
                     // (i.e. called _alarm->p() at wait_netxt(); different from
                     // "thread_last_preemption" since jobs can be preempted)
    Tick job_utilization; // accumulated execution time (in ticks)
    unsigned int average_job_execution_time; // average execution time of the
                                             // last 5 jobs.
    unsigned int number_dispatches; // number of dispatches that thread did
    unsigned int jobs_released; // number of jobs of a thread that were released
                                // so far (i.e. the number of times _alarm->v()
                                // was called by the Alarm::handler())
    unsigned int jobs_finished; // number of jobs of a thread that finished
                                // execution so far (i.e. the number of times
                                // alarm->p() was called at wait_next())
    unsigned int deadlines_missed;    
    unsigned int average_frequency;
    unsigned int num_charges;
  };
  ...
  struct Real_Statistics { // for Traits<System>::monitored = true
    ...
    unsigned int jobs_finished; // number of jobs of a thread that finished
                                ...
                                // alarm->p() was called at wait_next())
    unsigned int deadlines_missed;
    unsigned long long average_frequency;
    unsigned int num_charges;
  };

  typedef IF<Traits<Thread>::monitored, Real_Statistics,
             Dummy_Statistics>::Result Statistics;
...
public:
  template <typename... Tn> RR(int p = NORMAL, Tn &...an) : Priority(p) {}
};
...
public:
  template <typename... Tn> FCFS(int p = NORMAL, Tn &...an);
};
...
public:
  template <typename... Tn> GRR(int p = NORMAL, Tn &...an) : RR(p) {}
...
public:
  ...
  Fixed_CPU(int p = NORMAL, unsigned int cpu = ANY, Tn &...an)
      : Priority(p),
        Variable_Queue_Scheduler(
            ((_priority == IDLE) || (_priority == MAIN)) ? CPU::id()
            : (cpu != ANY)                               ? cpu
                           : ++_next_queue %= CPU::cores()) {}
...
protected:
  ...
  RT_Common(int i, Microsecond p, Microsecond d, Microsecond c)
      : Priority(i), _period(ticks(p)), _deadline(ticks(d ? d : p)),
        _capacity(ticks(c)) {}
...
public:
  ...
  RM(Microsecond p, Microsecond d = SAME, Microsecond c = UNKNOWN,
     unsigned int cpu = ANY)
      : RT_Common(int(ticks(p)), p, d, c) {}
};
...
public:
  ...
  DM(Microsecond p, Microsecond d = SAME, Microsecond c = UNKNOWN,
     unsigned int cpu = ANY)
      : RT_Common(int(ticks(d ? d : p)), p, d, c) {}
};
...
public:
  ...
  LM(Microsecond p, Microsecond d, Microsecond c, unsigned int cpu = ANY)
      : RT_Common(int(ticks((d ? d : p) - c)), p, d, c) {}
};
...
public:
  EDF_Modified(int p = APERIODIC)
      : RT_Common(p), _min_frequency(CPU::max_clock()),
        _max_frequency(CPU::max_clock()), _last_deadline(0), _step(-1) {}
  EDF_Modified(Microsecond p, Microsecond d, Microsecond c,
               int task_type = CRITICAL);
...
public:
  ...
  template <typename... Tn>
  GEDF_Modified(Microsecond p, Microsecond d, Microsecond c,
                int task_type = CRITICAL, Tn &...an)
...
// Scheduling Queues
template <typename T>
class Scheduling_Queue<T, GRR> : public Multihead_Scheduling_List<T> {};

template <typename T>
class Scheduling_Queue<T, GEDF_Modified> : public Multihead_Scheduling_List<T> {
};

template <typename T>
class Scheduling_Queue<T, Fixed_CPU> : public Scheduling_Multilist<T> {};

template <typename T>
class Scheduling_Queue<T, PEDF_Modified> : public Scheduling_Multilist<T> {};

template <typename T>
class Scheduling_Queue<T, CPU_Affinity> : public Scheduling_Multilist<T> {};
