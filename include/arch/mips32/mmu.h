...
class MIPS32_MMU: public MMU_Common<0, 0, 0>
{
private:
...
public:
    ...
    public:
        DMA_Buffer(unsigned int s) : Chunk(s, 0) {
            ...
            db<MIPS32_MMU>(TRC) << "MIPS32_MMU::DMA_Buffer() => " 
                              << *this << "\n";
        }
        DMA_Buffer(unsigned int s, const Log_Addr & d)
            ...
            db<MIPS32_MMU>(TRC) << "MIPS32_MMU::DMA_Buffer(phy=" << *this 
                              << " <= " << d <<"\n";
        }
...
public:
    ...
    static Phy_Addr alloc(unsigned int bytes = 1) {
        db<MIPS32_MMU>(TRC) << "MIPS32_MMU::alloc(bytes=" << bytes <<")\n";
        Phy_Addr phy(false);
        if(bytes) {
    ...
    static void free(Phy_Addr addr, int n = 1) {
        db<MIPS32_MMU>(TRC) << "MIPS32_MMU::free(addr=" << (void *)addr
                          << ",n=" << n << ")\n";
        if(addr && n) {
