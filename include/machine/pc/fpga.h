...
public:
    static void reset() {
        ...
        reg(WDMATLPA) = _dma_buf;           // Write: Write DMA TLP Address register with starting address
        ...
        reg(RDMATLPA) = _dma_buf + 4 * 1024; // Write: Read DMA TLP Address register with starting address.
...
// PC FPGA
class PC_FPGA: private FPGA_Common, private XilinxXXX
{
    friend class PC;
