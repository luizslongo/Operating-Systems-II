...
class TCP::Header {
 ...
 public:
    ...
    u16 dst_port() const { return ntohs(_dst_port); }
    u16 src_port() const { return ntohs(_src_port); }
    u32 seq_num() const { return ntohl(_seq_num); }
    u32 ack_num() const { return ntohl(_ack_num); }
    u16 wnd() const { return ntohs(_wnd); }
    u16 chksum() const { return ntohs(_chksum); }
    u16 urgptr() const { return ntohs(_urgptr); }
    
    // setters
    void seq_num(u32 v) { _seq_num = htonl(v); }
    void ack_num(u32 v) { _ack_num = htonl(v); }
    void wnd(u16 v) { _wnd = htons(v); }
    void chksum(u16 v) { _chksum = htons(v); }
    void urgptr(u16 v) { _urgptr = htons(v); }
    void dst_port(u16 v) { _dst_port = htons(v); }
    void src_port(u16 v) { _src_port = htons(v); }
